...
type UsedCapabilities struct {
	ArraySerialize bool
	InterfaceSerialize bool
	CustomTypeSerialize bool
}
...
func GenSerializationHeader(serializerName, typeName string, capabilities UsedCapabilities) string {
	header := fmt.Sprintf(`
...
func GenFieldSerialization(serializerName string, fieldInfo StructField, capabilities *UsedCapabilities) string {
	...
	if fieldInfo.isArray {
		capabilities.ArraySerialize = true
	...
	if fieldInfo.typeName == "interface" {
		capabilities.InterfaceSerialize = true
	...
	if fieldInfo.isCustomType {
		capabilities.CustomTypeSerialize = true
		return fmt.Sprintf(`
...
func GenUnserializationHeader(serializerName, typeName string, capabilities UsedCapabilities) string {
	header := fmt.Sprintf(`
	...
	if capabilities.InterfaceSerialize {
		header += `
			var tempCustom interface{}
			var tempIdentifier uint16
		`
	} else if capabilities.CustomTypeSerialize {
		header += `
			var tempCustom interface{}
		`
	}
