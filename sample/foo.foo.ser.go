...
func (self Foo) HashingSerialize() ([]byte, error) {
	...
	tempHeader = standard.NewIdentifierHeader(TypeIdTable[self.CustomInterface.(standard.Namer).Name()])
	bytesTemp, _ = tempHeader.Serialize()
	output = append(output, bytesTemp...)

	if tempHeader.Is16BitIdentifier {
		output = append(output, encoders.Uint16AsBytes(TypeIdTable[self.CustomInterface.(standard.Namer).Name()])...)
	} else {
		output = append(output, encoders.Uint8AsBytes(uint8(TypeIdTable[self.CustomInterface.(standard.Namer).Name()]))...)
	}

	bytesTemp, _ = self.CustomInterface.(HashingSerializer).HashingSerialize()
	output = append(output, bytesTemp...)
	return output, nil
}
func (self Foo) HashingUnserialize(data []byte) (interface{}, uint64, error) {
	...
	index += consumed
	output.CustomInterface = tempCustom
...
func (self Foo) Serialize() ([]byte, error) {
	...
	bytesTemp, _ = self.CustomInterface.(standard.Serializer).Serialize()
	output = append(output, bytesTemp...)
	return output, nil
}
